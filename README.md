...
Мой переносимый конфиг для [pi](https://pi.dev): глобальные инструкции, кастомные агенты, расширения, prompt-шаблоны, standalone-скиллы, MCP и настройки интерфейса.
...
Скрипт копирует только переносимую конфигурацию в `~/.pi/agent` и не трогает локальные credentials, сессии и прочее состояние. Standalone-скиллы из `config/skills` устанавливаются поверх существующих: скиллы пользователя, которых нет в репозитории, не удаляются. Путь можно переопределить переменной `PI_CODING_AGENT_DIR`.
...
Экспорт обновляет только переносимые файлы, включая standalone-скиллы из `~/.pi/agent/skills`, и специально не забирает credentials, сессии и кеши. Содержимое `~/.agents/skills` отдельно не экспортируется. После этого проверьте изменения и отправьте их в Git:
